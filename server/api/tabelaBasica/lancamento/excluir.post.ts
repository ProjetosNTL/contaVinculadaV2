...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const selectQuery = `SELECT descricao FROM tabelaBasica.lancamento WHERE codigo = ${codigo}`
    ...
    const queryExec = `EXEC tabelaBasica.lancamento_Atualiza ${codigo}, ${descSql}, ${usuario}, 0`
