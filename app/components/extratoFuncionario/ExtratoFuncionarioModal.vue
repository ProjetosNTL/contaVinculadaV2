<template>
  <AppModal :isOpen="isOpen" title="Extrato do Funcionário" @close="fechar" size="4xl">
    <div class="p-4 bg-gray-50 border rounded-md mb-4 shadow-sm">
      ...
      <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
        ...
        <div>
           <button @click="buscarDetalhes" class="w-full bg-blue-600 text-white rounded p-1.5 text-sm hover:bg-blue-700 flex justify-center items-center gap-2">
             <Icon name="fa-solid:search" /> Filtrar
           </button>
        </div>
      </div>
    </div>
      ...
      <table class="w-full text-left border-collapse whitespace-nowrap text-sm">
        <thead class="bg-[#2F5597] text-white sticky top-0 z-10">
          <tr>
            ...
            <th class="p-2 border-b text-right">Saldo</th>
          </tr>
        </thead>
        <tbody>
          <tr v-if="carregando">
            <td colspan="5" class="p-8 text-center text-gray-500 font-bold">Buscando extrato...</td>
          </tr>
          <tr v-else-if="detalhes.length === 0">
            <td colspan="5" class="p-8 text-center text-gray-500">Nenhum lançamento encontrado para este filtro.</td>
          </tr>
          <tr v-for="(item, idx) in detalhes" :key="idx" class="hover:bg-gray-100 border-b even:bg-gray-50">
            ...
            <td class="p-2 truncate max-w-[250px]" :title="item.detalhes || item.tipoLancamentoDesc">
...
const buscarDetalhes = async () => {
  ...
  try {
    const response = await $fetch<{ data: any[] }>('/api/operacao/movimentacaoBancaria/extratoFuncionario/detalhes', {
      method: 'POST',
