  ...
  <div class="p-6">
    ...
    <div class="bg-white rounded-lg shadow-md mb-6 p-4">
      ...
      <div v-show="filtroAberto" class="mt-4 flex justify-between">
        ...
        <button @click="buscarStatus" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 flex items-center">
          <span>Filtrar</span>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
      <table class="w-full text-left border-collapse">
        ...
        <tbody>
          <tr v-if="listaStatus.length === 0">
            <td colspan="3" class="p-4 text-center text-gray-500 font-bold">Nenhum status encontrado</td>
          </tr>
          <tr v-for="item in listaStatus" :key="item.codigo" class="hover:bg-gray-50 border-b">
            <td class="p-3">
              <NuxtLink :to="`/tabelaBasica/status/cadastro?id=${item.codigo}`" class="text-blue-600 hover:underline">
                {{ item.descricao }}
              </NuxtLink>
            </td>
    ...
    <AppModal :isOpen="modalHistoricoAberto" title="Histórico de alterações do Status" @close="modalHistoricoAberto = false">
...
const abrirHistorico = async (id: number) => {
  try {
    const response = await $fetch<{ data: Historico[] }>('/api/tabelaBasica/status/historico', {
      method: 'POST',
      body: { status: id }
    })
