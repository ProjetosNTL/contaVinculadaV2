<template>
  <div class="p-6">
      ...
      <table class="w-full text-left border-collapse">
        ...
        <tbody>
          <tr v-if="lista.length === 0">
            <td colspan="3" class="p-4 text-center text-gray-500 font-bold">Nenhum lançamento encontrado</td>
          </tr>
          <tr v-for="item in lista" :key="item.codigo" class="hover:bg-gray-50 border-b">
            <td class="p-3">
              <NuxtLink :to="`/tabelaBasica/lancamento/cadastro?id=${item.codigo}`" class="text-blue-600 hover:underline">
                {{ item.descricao }}
              </NuxtLink>
            </td>
    ...
    <AppModal :isOpen="modalHistoricoAberto" title="Histórico de alterações do Lançamento" @close="modalHistoricoAberto = false">
