    ...
    <div class="bg-white rounded-lg shadow-md mb-6 p-4">
      ...
      <div v-show="filtroAberto" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select v-model="filtro.contaVinculada" class="w-full border rounded-md p-2 bg-white">
            <option value="">Todas</option>
            <option v-for="conta in contas" :key="conta.codigo" :value="conta.codigo">
              {{ conta.nomeBanco }}
            </option>
          </select>
        </div>
      ...
      <div v-show="filtroAberto" class="mt-4 flex justify-between">
        <button @click="router.push('/operacao/movimentacaoBancaria/extratoFuncionario')" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 flex items-center">
          <span>Extrato Funcionário</span>
        </button>
      ...
      <table class="w-full text-left border-collapse whitespace-nowrap">
        <thead class="bg-gray-100">
          <tr>
            ...
            <th class="p-3 border-b text-center">Banco</th>
            <th class="p-3 border-b text-center">Saldo</th>
            <th class="p-3 border-b text-center">Extrato Projeto</th>
          </tr>
        </thead>
        <tbody>
          <tr v-if="lista.length === 0">
            <td colspan="4" class="p-4 text-center text-gray-500 font-bold">Nenhum projeto encontrado</td>
          </tr>
          <tr v-for="item in lista" :key="item.codigoProjeto" class="hover:bg-gray-50 border-b">
            <td class="p-3 text-center">{{ item.apelido }} - {{ item.projeto }}</td>
            <td class="p-3 text-center">{{ item.nomeBanco || '-' }}</td>
            <td class="p-3 text-center text-lg font-bold" :class="item.saldoProjeto >= 0 ? 'text-green-600' : 'text-red-600'">
              R$ {{ formatarMoeda(item.saldoProjeto) }}
            </td>
            <td class="p-3 text-center">
              <button @click="abrirModalExtrato(item.codigoProjeto)" class="bg-blue-500 text-white p-2 rounded hover:bg-blue-600">
...
const filtro = ref({ projeto: '', contaVinculada: '' })
const lista = ref<any[]>([])
...
const carregarCombos = async () => {
  try {
    ...
    const resBancos = await $fetch<any[]>('/api/tabelaBasica/bancos/ativos')
    contas.value = resBancos || []
  } catch (error) {
...
const buscarSaldos = async () => {
  try {
    const response = await $fetch<{ data: any[] }>('/api/operacao/movimentacaoBancaria/extratoProjeto/listagem', {
