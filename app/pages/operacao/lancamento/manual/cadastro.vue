<template>
  <div class="h-full flex flex-col gap-6">
    <div class="flex items-center justify-between">
      ...
      <button @click="navigateTo('/operacao/lancamento/manual')" class="bg-gray-500 dark:bg-gray-700 hover:bg-gray-600 dark:hover:bg-gray-600 text-white px-4 py-2 rounded-sm font-medium transition flex items-center gap-2 shadow-sm">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-sm border border-gray-200 dark:border-gray-700 p-5 shadow-sm transition-colors duration-300">
      <div class="flex items-center gap-2 mb-4 border-b border-gray-200 dark:border-gray-700 pb-3">
        ...
        <h2 class="font-semibold text-gray-700 dark:text-gray-200">Dados do Lançamento</h2>
      </div>
      ...
      <form @submit.prevent="confirmarGravacao" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Tipo Movimentação <span class="text-red-500">*</span></label>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Valor da Movimentação <span class="text-red-500">*</span></label>
            <div class="relative">
              <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><Icon name="fa7-solid:brazilian-real-sign" class="text-gray-400 dark:text-gray-500" /></div>
              <input v-model="form.valorStr" @blur="formatarMoeda" @focus="desformatarMoeda" type="text" placeholder="0,00" class="w-full bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-sm pl-8 pr-3 py-2 text-gray-800 dark:text-gray-200 focus:outline-none focus:border-[#3276b1] dark:focus:border-[#539ce0] transition text-right" />
            </div>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Data da Movimentação <span class="text-red-500">*</span></label>
            <div class="relative">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Motivo <span class="text-red-500">*</span></label>
          <textarea v-model="form.motivo" placeholder="Descreva o motivo deste lançamento..." class="w-full border border-gray-300 dark:border-gray-600 rounded-sm p-3 text-gray-800 dark:text-gray-200 focus:outline-none focus:border-[#3276b1] dark:focus:border-[#539ce0] resize-y min-h-[100px] bg-[#ffffc0] dark:bg-[#4a4a15] dark:placeholder-gray-400 transition"></textarea>
        </div>
        ...
        <div class="mt-8">
          ...
          <div class="flex flex-col md:flex-row gap-4 items-end">
            ...
            <div class="flex gap-2">
              <button type="button" @click="addFuncionario" class="bg-[#3276b1] dark:bg-[#539ce0] hover:bg-[#275b89] dark:hover:bg-[#3276b1] text-white px-4 py-2 rounded-sm font-medium transition flex items-center gap-2">
                <Icon name="fa7-solid:plus" class="w-4 h-4" /> Adicionar
              </button>
              <button type="button" @click="removerSelecionados" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-sm font-medium transition flex items-center gap-2">
                <Icon name="fa7-solid:minus" class="w-4 h-4" /> Remover
              </button>
            </div>
          </div>
      ...
      <div v-if="modalConf" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="modalConf = false">
        <div class="bg-white dark:bg-gray-800 rounded-md shadow-2xl w-full max-w-lg overflow-hidden border border-gray-200 dark:border-gray-700 flex flex-col">
          <div class="p-8 text-center space-y-4">
            ...
            <h2 class="text-xl font-medium text-gray-800 dark:text-gray-200">Tem certeza que deseja aplicar esse lançamento para todos os funcionários do projeto?</h2>
          </div>
...
const form = reactive({
  ...
  valorStr: '', valorMovimentacao: 0, dataMovimentacao: '', motivo: '',
  funcionarios: [] as any[]
})
...
const formatarMoeda = () => {
  let val = parseFloat(form.valorStr.replace(/\./g, '').replace(',', '.'))
  if (isNaN(val)) val = 0
  form.valorMovimentacao = val
  form.valorStr = val.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
}
const desformatarMoeda = () => { if (form.valorMovimentacao) form.valorStr = form.valorMovimentacao.toString().replace('.', ','); else form.valorStr = '' }
...
const confirmarGravacao = () => {
  ...
  if (form.valorMovimentacao <= 0) return alert("Informe um Valor maior que zero.")
...
const efetivarGravacao = async () => {
  ...
  try {
    const resp: any = await $fetch('/api/operacao/lancamento/manual/grava', { method: 'POST', body: { ...form, funcionarios: form.funcionarios.filter((f: any) => f.tipoAlteracao !== 2) } })
    if (resp.status === 'success') { alert("Operação Gravada com Sucesso"); navigateTo('/operacao/lancamento/manual') }
...
const limpar = () => {
  form.projeto = ''; form.contaVinculada = ''; form.tipoMovimentacao = ''; form.valorStr = ''; form.valorMovimentacao = 0
  form.dataMovimentacao = ''; form.motivo = ''; form.funcionarios = []; pesquisaFuncs.value = ''; carregarContas()
}
