<template>
  <div class="h-full flex flex-col gap-6">
    <div class="flex items-center justify-between">
      ...
      <button @click="navigateTo('/operacao/lancamento/reembolso/cadastro')" class="bg-[#3276b1] dark:bg-[#539ce0] hover:bg-[#275b89] dark:hover:bg-[#3276b1] text-white px-4 py-2 rounded-sm font-medium transition flex items-center gap-2 shadow-sm">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-sm border border-gray-200 dark:border-gray-700 p-5 shadow-sm transition-colors duration-300">
      ...
      <form @submit.prevent="filtrar" class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-4">
        ...
        <div class="col-span-1">
          ...
          <div class="relative">
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><Icon name="fa7-solid:calendar" class="text-gray-400 dark:text-gray-500" /></div>
    ...
    <div class="flex-1 bg-white dark:bg-gray-800 rounded-sm border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden flex flex-col transition-colors duration-300">
      <div v-if="loading" class="flex-1 flex items-center justify-center p-12 text-gray-500 dark:text-gray-400">
        <Icon name="fa7-solid:spinner" class="animate-spin w-10 h-10 text-[#3276b1] dark:text-[#539ce0]" />
      </div>
      <div v-else-if="lancamentos.length === 0 && buscou" class="flex-1 flex flex-col items-center justify-center p-12 text-gray-400 dark:text-gray-500">
        <p class="text-lg">Nenhum lançamento encontrado.</p>
      </div>
      ...
      <div v-else-if="lancamentos.length > 0" class="flex-1 overflow-x-auto">
        <table class="w-full text-left border-collapse whitespace-nowrap text-gray-700 dark:text-gray-300">
          <thead>
            <tr class="bg-gray-100/70 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 text-sm text-gray-700 dark:text-gray-400">
              <th class="p-3 font-semibold">Projeto</th>
              <th class="p-3 font-semibold">Conta V.</th>
              <th class="p-3 font-semibold">Tipo Mov.</th>
              <th class="p-3 font-semibold text-center">Data Mov.</th>
              <th class="p-3 font-semibold text-right">Valor Mov.</th>
              <th class="p-3 font-semibold text-center">Data Ofício</th>
              <th class="p-3 font-semibold text-right">Valor Ofício</th>
              <th class="p-3 font-semibold text-center">Data Resp.</th>
              <th class="p-3 font-semibold text-center">Data Entr.</th>
              <th class="p-3 font-semibold">Status</th>
              <th class="p-3 font-semibold text-center">Funcs.</th>
              <th class="p-3 font-semibold text-center">Detalh.</th>
            </tr>
          </thead>
          <tbody class="text-sm">
            <tr v-for="lan in lancamentos" :key="lan.codigo" class="border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
              <td class="p-2 text-gray-800 dark:text-gray-200">{{ lan.projeto }}</td>
              <td class="p-2 text-gray-600 dark:text-gray-400"><span class="truncate block max-w-[120px]">{{ lan.contaVinculada }}</span></td>
              <td class="p-2 text-gray-600 dark:text-gray-400">{{ lan.tipoMovimentacao }}</td>
              <td class="p-2 text-gray-600 dark:text-gray-400 text-center">{{ formatarData(lan.dataMovimentacao) }}</td>
              <td class="p-2 text-right font-medium text-blue-600 dark:text-blue-400">R$ {{ formatarValor(lan.valorMovimentacao) }}</td>
              <td class="p-2 text-gray-600 dark:text-gray-400 text-center">{{ formatarData(lan.dataOficio) }}</td>
              <td class="p-2 text-right">R$ {{ formatarValor(lan.valorOficio) }}</td>
              <td class="p-2 text-gray-600 dark:text-gray-400 text-center">{{ formatarData(lan.dataResposta) }}</td>
              <td class="p-2 text-gray-600 dark:text-gray-400 text-center">{{ formatarData(lan.dataEntrada) }}</td>
              <td class="p-2 text-gray-600 dark:text-gray-400">{{ lan.status }}</td>
              <td class="p-2 text-center">
                <button @click="abrirModalFuncionario(lan)" class="bg-[#3276b1] dark:bg-[#539ce0] hover:bg-[#275b89] dark:hover:bg-[#437fb8] text-white w-7 h-7 rounded-sm inline-flex items-center justify-center disabled:opacity-30 disabled:cursor-not-allowed transition" :disabled="lan.funcionario !== 1">
                  <Icon name="fa7-solid:user" class="w-3 h-3" />
                </button>
              </td>
              <td class="p-2 text-center">
                <button @click="abrirModalDetalhes(lan)" class="bg-[#3276b1] dark:bg-[#539ce0] hover:bg-[#275b89] dark:hover:bg-[#437fb8] text-white w-7 h-7 rounded-sm inline-flex items-center justify-center transition">
                  <Icon name="fa7-solid:list" class="w-3 h-3" />
                </button>
              </td>
            </tr>
          </tbody>
    ...
    <Teleport to="body">
      <div v-if="modalFunc" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="modalFunc = false">
        <div class="bg-white dark:bg-gray-800 rounded-md shadow-2xl w-full max-w-2xl overflow-hidden border border-gray-200 dark:border-gray-700 flex flex-col">
          <div class="bg-[#22262e] dark:bg-[#1a1c23] px-4 py-3 flex justify-between items-center text-white">
            <h3 class="font-medium text-lg">Funcionários do Reembolso</h3>
          ...
          <div class="p-6 overflow-y-auto max-h-[60vh] bg-white dark:bg-gray-900">
            ...
            <table v-else class="w-full text-left border-collapse border border-gray-200 dark:border-gray-700">
              <tbody>
                <tr v-for="(f, i) in currentFuncs" :key="i" class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                  <td class="p-3 text-gray-800 dark:text-gray-200 text-center font-medium">{{ f.funcionario }}</td>
                </tr>
                <tr v-if="currentFuncs.length === 0"><td class="p-4 text-center text-gray-500 dark:text-gray-400">Nenhum funcionário vinculado.</td></tr>
              </tbody>
            </table>
          </div>
          <div class="bg-gray-50 dark:bg-[#22262e] px-4 py-3 border-t border-gray-200 dark:border-gray-700 flex justify-end">
      ...
      <div v-if="modalDet" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="modalDet = false">
        <div class="bg-white dark:bg-gray-800 rounded-md shadow-2xl w-full max-w-3xl overflow-hidden border border-gray-200 dark:border-gray-700 flex flex-col">
          <div class="border-b border-gray-200 dark:border-gray-700 px-4 py-3 flex justify-between items-center bg-gray-50 dark:bg-gray-900">
            <h3 class="font-bold text-lg text-gray-700 dark:text-gray-200">Detalhes do Reembolso</h3>
            <button @click="modalDet = false" class="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition"><Icon name="fa7-solid:xmark" class="w-5 h-5" /></button>
          </div>
          ...
          <div class="bg-gray-50 dark:bg-gray-900 px-4 py-4 border-t border-gray-200 dark:border-gray-700 flex justify-center">
            <button @click="modalDet = false" class="bg-[#3276b1] dark:bg-[#539ce0] hover:bg-[#275b89] dark:hover:bg-[#437fb8] text-white px-8 py-2 rounded-sm font-bold shadow-sm transition">Fechar</button>
          </div>
        </div>
      </div>
    </Teleport>
...
const formatarValor = (valor: number) => { if (valor === null || valor === undefined) return ''; return valor.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }

const filtrar = async () => {
  loading.value = true; buscou.value = true
  try {
    const resp: any = await $fetch('/api/operacao/lancamento/reembolso/listagem', { method: 'POST', body: filter })
    lancamentos.value = resp?.results || []
  } catch (e) { console.error(e); alert('Falha ao buscar lançamentos de reembolso') } finally { loading.value = false }
}

const abrirModalFuncionario = async (lanc: any) => {
  modalFunc.value = true; loadingFunc.value = true; currentFuncs.value = []
  try {
    const resp: any = await $fetch('/api/operacao/lancamento/reembolso/recuperaFuncionario', { method: 'POST', body: { lancamentoReembolso: lanc.codigo } })
