  ...
  <div class="h-full flex flex-col gap-6">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-normal text-gray-700 dark:text-gray-200 flex items-center gap-2">
        <Icon name="fa7-solid:list" class="w-6 h-6 text-[#3276b1] dark:text-[#539ce0]" />
        Extrato Funcionário
      </h1>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-sm border border-gray-200 dark:border-gray-700 p-5 shadow-sm transition-colors duration-300">
      <div class="flex items-center gap-2 mb-4 border-b border-gray-200 dark:border-gray-700 pb-3">
        ...
        <h2 class="font-semibold text-gray-700 dark:text-gray-200">Filtro de Extrato</h2>
      </div>
      <form @submit.prevent="filtrar" class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-4">
        ...
        <div class="col-span-1 lg:col-span-1">
          <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Funcionário</label>
          <div class="relative">
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><Icon name="fa7-solid:user" class="text-gray-400 dark:text-gray-500" /></div>
            <input type="text" v-model="filter.funcionarioNome" placeholder="Digite o nome..." class="w-full bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-sm pl-10 pr-3 py-2 text-gray-800 dark:text-gray-200 focus:outline-none focus:border-[#3276b1] dark:focus:border-[#539ce0] transition placeholder-gray-400 dark:placeholder-gray-500" />
          </div>
        </div>
    ...
    <div class="flex-1 bg-white dark:bg-gray-800 rounded-sm border border-gray-200 dark:border-gray-700 shadow-sm min-h-[400px] transition-colors duration-300">
      ...
      <div v-else-if="!buscou" class="flex flex-col items-center justify-center p-12 text-gray-400 dark:text-gray-500">
        ...
        <p>Preencha os filtros para carregar o extrato do funcionário.</p>
      </div>
      <div v-else class="flex flex-col items-center justify-center p-12 text-gray-500 dark:text-gray-400">
        ...
        <p class="text-sm">O módulo backend de extratos deste painel ainda não foi concluído.</p>
      </div>
    </div>
  </div>
</template>
...
const filter = reactive({ projeto: '', funcionarioNome: '', dataInicial: '', dataFinal: '' })
